...
isa_anno |>
  left_join(
    sample_sheet |>
      dplyr::select(
        sample_name, species, sample_source, gene_marking_percent
      ) |>
      distinct(),
    by="sample_name"
  ) |>
  dplyr::select(
    sample_name, seqnames, start, end, width, strand, score,
    annotation_detailed=annotation, tss_distance=distanceToTSS,
    gene_seqnames=geneChr, gene_start=geneStart, gene_end=geneEnd,
    gene_width=geneLength, gene_strand=geneStrand, gene_id=geneId,
    transcript_id=transcriptId, species, sample_source, gene_marking_percent
  ) |>
  mutate(
    annotation_simple=str_remove(annotation_detailed, "\\s\\(.+$"),
    sample_name=factor(
      sample_name, levels=str_sort(unique(sample_name), numeric=TRUE)
    ),
    seqnames=factor(seqnames, levels=str_sort(unique(seqnames), numeric=TRUE))
  ) |>
  dplyr::relocate(annotation_simple, .after=annotation_detailed) |>
  arrange(sample_name, desc(score), seqnames, start) |>
