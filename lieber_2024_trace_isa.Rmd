...
Return the sum of valies for the group of technical replicates.
...
Generate a bar graph of the number of insertion sites per sample. This is
defined as the number of unique genomic locations for each sample where an
insertion event was detected.
...
Multiple unique sequenced fragments can end up mapping to the same genomic
location in the same sample. This is likely due to the clonal expansion of a
cell with a particular genomic insertion site. This bargraph represents all
fragments of DNA with insertion sites sequenced for each sample regardless of
genomic location.
...
As described above, a sample can have multiple DNA fragments with insertion
sites mapping to the same genomic location. This stacked barplot represents
for each genomic location with an insertion site in the sample,
the fraction of insertion-site containing DNA fragments sequenced associated
with that site.
...
-   **Promoter**: -2,000 to +200 bp relative to *transcript* transcription
start sites (TSS).
...
-   **3\` UTR**: From the end of the ORF to the last exon of a transcript.

-   **Downstream**: +3,000 kbp from the end of the last exon of the transcript.
...
The distance to the nearest TSS for each insertion site is calculated as part
of the annotation process. Here we create a histogram of these distances
for each sample, excluding insertion sites greater than +500 kbp or less than
-500 kbp from the nearest TSS.
...
create a sequence logo centered around the insertion site including ±20 bps.

Sleeping beauty is known to have a palindromic motif at the insertion site with
A(-3), T(0), A(+1), and T(+4).
